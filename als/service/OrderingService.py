...
class Analyzer:
    ...
    def past_loans_metadata(self):
        end_ts = date_to_ts(dt.datetime.utcnow())
    ...
    def running_loans_metadata(self):
        ...
        if active_loans:
            ...
            latest_ts = np_date_to_ts(latest['date'].get_values()[0])
    ...
    def evaluate_state(self, adjusted):
        ...
        now_ts = date_to_ts(dt.datetime.utcnow())
        ...
        if running_loans_metadata:
            ...
            close_of_latest_ts = date_to_ts(dt.datetime.strptime(past_loans_metadata['close_of_latest'], time_format))
    ...
    def signal_to_place(self):
        if self._adjusted_loans['possible_loans'] < 1:
            return False
        minutes = 26 * check_interval
        now = date_to_ts(dt.datetime.utcnow())
